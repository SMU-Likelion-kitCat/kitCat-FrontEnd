...
// 루틴 주차별 기록 조회
export const getRoutineWeekDetail = async (year, month) => {
  try {
    const res = await apiConfig.get(`/routine/month/${year}/${month}`)
    return res.data
  } catch (e) {
    console.error(e)
    throw e
...
// 선택한 산책 전체 루틴 조회
export const getRoutineWalkDetail = async (routineId) => {
  try {
